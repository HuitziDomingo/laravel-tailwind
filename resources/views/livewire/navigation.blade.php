      ...
      @auth
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
            ...
            <div x-show="open" x-on:click.away="open = false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                ...
                <a href="{{route('admin.home')}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Panel de Control</a>
  ...
  <div x-show="open" x-on:click.away="open = false" class="sm:hidden">
    <div class="px-2 pt-2 pb-3 space-y-1">
      ...
      @foreach ($categories as $cate)
        <a href="{{route('posts.category', $cate)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{$cate->name}}</a>
      @endforeach
    </div>
  </div>
</nav>
